...
def get_default_classifiers_grid_search(
    ...
    if mnb_key in types:
        mnb_param = [{'alpha': np.logspace(-10, 1, gs_steps)}]  # Corrected range for alpha
        mnb = MultinomialNB()
    ...
    if knn_key in types:
        knn_param = [
            {'n_neighbors': np.arange(1, 51)}  # Use valid parameter
        ]
    ...
    if dt_key in types:
        dt_param = [
            {'min_samples_leaf': np.linspace(1, 50, gs_steps).astype(int), 'max_depth': np.linspace(1, 50, gs_steps).astype(int)}  # Added max_depth
        ]
    ...
    if rf_key in types:
        rf_param = [
            {'n_estimators': np.linspace(100, 1000, gs_steps).astype(int), 'min_samples_leaf': np.linspace(1, 50, gs_steps).astype(int)}  # Added min_samples_leaf
        ]
    ...
    if ab_key in types:
        ab_param = [
            {'n_estimators': np.linspace(100, 2000, gs_steps).astype(int)},
    ]
        ab = ensemble.AdaBoostClassifier(algorithm='SAMME', random_state=random_state)  # Use SAMME instead of SAMME.R
...
def load_dataset(
    ...
    if dataset_sample_type.lower() == 'normal':
        ...
        raise ValueError('Unknown dataset sampling type')
